...
// settings - MIGLIORATI PER PRESTAZIONI E QUALITA'
const pixelate = 0;
...
// setup - MIGLIORATO PER QUALITA' GRAFICA
const laneWidth = 1400;            // how wide is track
const trackSegmentLength = 100;    // length of each segment
const drawDistance = 1200;         // AUMENTATO: più segmenti visibili (era 1e3)
...
function gameInit()
{
    ...
    // Inizializza variabili essenziali per evitare errori undefined
    time = frame = frameTimeLastMS = averageFPS = frameTimeBufferMS = 0;
    cameraPos = vec3();
    cameraRot = vec3();
    cameraOffset = 0;
    vehicles = [];
...
function gameStart()
{
    ...
    vehicles.push(playerVehicle = new PlayerVehicle(testStartZ?testStartZ:playerStartZ, playerColor));

    // Aggiungi i veicoli dei giocatori remoti se in multiplayer
    if (multiplayerEnabled && remotePlayers) {
        remotePlayers.forEach((remotePlayer) => {
            if (remotePlayer.vehicle && !remotePlayer.vehicle.destroyed) {
                vehicles.push(remotePlayer.vehicle);
            }
        });
    }
...
function gameUpdate(frameTimeMS=0)
{
    ...
    for (;frameTimeBufferMS >= 0; frameTimeBufferMS -= 1e3/frameRate)
    {
        ...
        // Solo aggiorna il gioco se è stato avviato (playerVehicle esiste)
        if (playerVehicle) {
            gameUpdateInternal();
            enhancedModeUpdate();
            debugUpdate();
            ...
            // Se non c'è playerVehicle, aggiorna solo l'input per i menu
            inputUpdate();
            inputUpdatePost();
        }
    }
    ...
    // Solo renderizza se il gioco è avviato
    if (playerVehicle && track) {
        ...
        // Se siamo nel menu, renderizza solo un background
        glPreRender(mainCanvasSize);
    }
